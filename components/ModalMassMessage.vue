<template>
  <div class="z-2 w-screen h-screen absolute op flex items-center justify-center">
    <div class="h-max p-6 bg-base2 rounded-lg max-w-md w-full">
      <h2 class="text-white text-center text-2xl font-bold">Enter message</h2>
      <textarea ref="text" class="bg-base2 rounded-lg my-4 w-full" cols="30" rows="10" placeholder="message..."></textarea>
      <div class="flex flex-row justify-around text-white text-lg font-bold p-3 items-center">
        ...
        <div @click="confirm" @mousedown="mousedown" @mouseup="mouseup" class="bg-basegreen p-2 rounded-lg cursor-pointer text-center w-36">Send message</div>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    confirm() {
      if (this.$refs.text.value) {
        this.$emit('confirm', this.$refs.text.value)
